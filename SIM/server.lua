...
RegisterServerEvent('adam_sim:buy')
AddEventHandler('adam_sim:buy', function(item)
...
ESX.RegisterUsableItem('sim', function (sim, source)
    local xPlayer = ESX.GetPlayerFromId(source)
    local metadata = { sim = number, owner = xPlayer.identifier }
    exports.ox_inventory:Search('count', 'sim', metadata)
    local number = GeneratePhoneNumber()
    MySQL.Sync.execute("INSERT INTO `sim` VALUES ('"..number.."', '"..xPlayer.identifier .."')")
    TriggerClientEvent("gcPhone:myPhoneNumber", source,sim)
    TriggerClientEvent("gcPhone:UpdateNumber", source,sim)
end)
...
ESX.RegisterServerCallback('adam_sim:getSims', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('adam_sim:check')
AddEventHandler('adam_sim:check', function(num)
    local xPlayer = ESX.GetPlayerFromId(source)
